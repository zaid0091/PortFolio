  ...
  const TimelineSection = ({ title, items, dateColor }) => {
    ...
    return (
      <div className="mb-12" ref={ref}>
        <h3
          className={`reveal-zoom text-[1rem] font-bold font-mono mb-6 px-3 py-1 border-2 inline-block rounded ${visible ? 'is-visible' : ''}`}
          style={{
        ...
        <div className="relative pl-8">
          ...
          {items.map((item, i) => (
            <div
              key={i}
              className={`relative mb-8 last:mb-0 reveal ${visible ? 'is-visible' : ''} delay-${i + 1}`}
            >
              {/* Dot */}
              <div
