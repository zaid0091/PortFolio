...
const POSTS: BlogPost[] = [
  {
    ...
    color: "#ffd93d",
    summary:
      "Walking through the architecture decisions, auth setup, and deployment pipeline I used when building my first full-stack project.",
    content: `## The Problem
...
The full project is on my GitHub if you want to dig into the code.`,
  },
  {
    ...
    color: "#66d9ef",
    summary:
      "The design system, animation patterns, and performance tricks I picked up while building this exact site you're reading right now.",
...
Building your own portfolio teaches you more than any tutorial. You hit every real problem: responsive layout, state management, performance, deployment. It's the best project to have on your resume.`,
  },
  {
    ...
    color: "#ff6b9d",
    summary:
      "A practical comparison based on real projects. Not benchmarks — actual developer experience, pricing surprises, and when to use each.",
    ...
    md.split("\n").forEach((line, i) => {
      ...
      if (line.startsWith("## ")) {
        ...
        const parts = text.split(/(\*\*[^*]+\*\*)|(`[^`]+`)/).map((p, j) => {
          ...
          if (p.startsWith("`") && p.endsWith("`"))
            return (
              <code key={j} className="blog-inline-code">
                {p.slice(1, -1)}
              </code>
            );
          return p;
        ...
        blocks.push(
          <li key={i} className="blog-modal-li">
            {parts}
          </li>,
        );
      } else if (line.trim() === "") {
    ...
    >
      <div className="blog-modal-dialog">
          ...
          <div className="blog-modal-header-content">
            ...
            <div className="blog-modal-meta">
              ...
              <span
                ...
                style={{
                  background:
                    readProgress >= 1 ? "#a8e6cf" : "var(--bg-secondary)",
        ...
        <div className="blog-modal-body" ref={bodyRef}>
          <div className="blog-modal-content">{renderFull(post.content)}</div>
        </div>
      </div>
    </div>
  );
...
export default function Blog() {
  ...
  return (
    <section
      id="blog"
      ref={ref}
      style={{ background: "var(--bg)", padding: "40px 0 60px" }}
    >
      <div className="max-w-[1100px] mx-auto px-6">
        <h2 className={`section-title reveal ${visible ? "is-visible" : ""}`}>
          Blog / Devlog
        </h2>
